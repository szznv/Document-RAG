...
# Questions file (1 line = 1 question)
...
def iter_questions_from_txt(path: Path):
    """1 строка = 1 вопрос. Пустые строки игнорируются."""  # [web:992]
    if not path.exists():
        raise FileNotFoundError(f"Файл с вопросами не найден: {path.resolve()}")
...
if __name__ == "__main__":
    ...
    log(f"Читаю вопросы из файла: {QUESTIONS_PATH.resolve()}")
    for line_no, user_query in iter_questions_from_txt(QUESTIONS_PATH):
        ...
        print("\n" + "=" * 50)
        print(f"Вопрос #{line_no}: {user_query}")
        print("-" * 50)
        print(result["answer"])
        print("=" * 50)
